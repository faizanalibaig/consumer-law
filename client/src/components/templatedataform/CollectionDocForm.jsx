...
import { firstname, middlename, lastname, state, postalcode, city, address,dob, number, disputedCollection, disputedCollectionInstruction } from "../../slice/collectionslice";


const CollectionDocForm = () => {
 ...
 const handleSubmit = (e) => {
  ...
  if (isValid) {
    ...
    dispatch(disputedCollection({ disputedCollection: formData.disputedCollection }));

    dispatch(disputedCollectionInstruction({ disputedCollectionInstruction: formData.disputedCollectionInstruction }));

    console.log('Form submitted successfully');
    navigate('/form/template/collection-document')
  }else {
    setErrors((prevErrors) => ({
      ...prevErrors,
 ...
 return (
  <div className='h-auto flex flex-col items-center'>
     <section className='px-5 sm:px-0 sm:pt-10 lg:pt-0'>
        <h1 className='text-[28px] sm:text-[38px] lg:text-[45px] xl:text-[50px] font-bold py-5 sm:py-10 sm:px-[120px] lg:px-[150px] xl:px-[300px] sm:text-center'>Fill Out The Form For Collection Document</h1>
      </section>
      ...
      <form onSubmit={handleSubmit} className="">
       <div className='grid grid-cols-1 sm:grid-cols-3 w-full gap-3 py-3'>
          ...
          <div className='flex flex-col gap-2'>
            <input
              type='text'
              name='disputedCollection'
              placeholder='Disputed Collection'
              value={formData.disputedCollection}
              onChange={handleChange}
              className='h-[60px] border-2 border-black/20 px-5 text-lg rounded-[12px]'
              required
              />
            {errors.disputedCollection && <span className='text-red-500'>{errors.disputedCollection}</span>}
            </div>
            <div className='flex flex-col gap-2'>
            <input
              type='text'
              name='disputedCollectionInstruction'
              placeholder='Disputed Collection Instruction'
              value={formData.disputedCollectionInstruction}
              onChange={handleChange}
              className='h-[60px] border-2 border-black/20 px-5 text-lg rounded-[12px]'
              required
              />
            {errors.disputedCollectionInstruction && <span className='text-red-500'>{errors.disputedCollectionInstruction}</span>}
            </div>
          </div>
...
export default CollectionDocForm
