...
import { firstname, middlename, lastname, state, postalcode, city, address,dob, number, disputedInquiries, disputedInquiriesDate, disputedInquiriesReason, disputeInquiriesInstructions } from "../../slice/inquiryslice";
...
const InquiryForm = () => {
 ...
 const handleSubmit = (e) => {
  ...
  if (isValid) {
    ...
    dispatch(disputedInquiriesDate({ disputedInquiriesDate: formData.disputedInquiriesDate }));
    dispatch(disputedInquiriesReason({ disputedInquiriesReason: formData.disputedInquiriesReason }));
    dispatch(disputeInquiriesInstructions({ disputeInquiriesInstructions: formData.disputeInquiriesInstructions }));
    

    console.log('Form submitted successfully');
    navigate('/form/template/inquiry-document')
  }else {
    setErrors((prevErrors) => ({
      ...prevErrors,
 ...
 return (
  <div className='h-auto flex flex-col items-center'>
     <section className='px-5 sm:px-0 sm:pt-10 lg:pt-0'>
        <h1 className='text-[28px] sm:text-[38px] lg:text-[45px] xl:text-[50px] font-bold py-5 sm:py-10 sm:px-[120px] lg:px-[150px] xl:px-[300px] sm:text-center'>Fill Out The Form For Inquiry Document</h1>
      </section>
      ...
      <form onSubmit={handleSubmit} className="">
       <div className='grid grid-cols-1 sm:grid-cols-3 w-full gap-3 py-3'>
          ...
          <div className='flex flex-col gap-2'>
            ...
            <input
              type='date'
              name='disputedInquiriesDate'
              placeholder='Disputed Inquiries Date'
              value={formData.disputedInquiriesDate}
              max={new Date().toISOString().split('T')[0]} 
              onChange={handleChange}
              className='h-[60px] border-2 border-black/20 px-5 text-lg rounded-[12px]'
              required
              />
            <div>
              <h1>Disputed Inquiries Date</h1>{errors.disputedInquiriesDate && <span className='text-red-500'>{errors.disputedInquiriesDate}</span>}
            </div>
            </div>
          </div>
          ...
          <div className='flex flex-col gap-2'>
            <input
              type='text'
              name='disputedInquiriesReason'
              placeholder='Disputed Inquiries Reason'
              value={formData.disputedInquiriesReason}
              onChange={handleChange}
              className='h-[60px] border-2 border-black/20 px-5 text-lg rounded-[12px]'
              required
              />
            {errors.disputedInquiriesReason && <span className='text-red-500'>{errors.disputedInquiriesReason}</span>}
            </div>
            <div className='flex flex-col gap-2'>
            <input
              type='text'
              name='disputeInquiriesInstructions'
              placeholder='Dispute Inquiries Instructions'
              value={formData.disputeInquiriesInstructions}
              onChange={handleChange}
              className='h-[60px] border-2 border-black/20 px-5 text-lg rounded-[12px]'
              required
              />
            {errors.disputeInquiriesInstructions && <span className='text-red-500'>{errors.disputeInquiriesInstructions}</span>}
            </div>
          </div>
